module BoxHelper
  ...
  def current_time
    if (current_user.identity == "1")
      @current_time = Limit.find(current_major?.to_i).student_time
    elsif (current_user.identity == "2")
      @current_time = Limit.find(current_major?.to_i).gda_time
    end
  end

  def final_time
    if (current_user.identity == "1")
      @final_time = Limit.find(current_major?.to_i).student_time_
    elsif (current_user.identity == "2")
      @final_time = Limit.find(current_major?.to_i).gda_time_
    end
  end
  ...
  def user_apply?
    if current_user.major == "응용화학과"
      if current_user.cabinet && current_user.cabinetApliSci
        redirect_to new_post_path, method: "get"
        flash[:alert] = "이미 신청하셨습니다."
      end
    elsif !((current_user.major == "응용화학과") || (current_user.major == "산업경영공학과"))
